...
"}")
        ...
        # Only hide error frames 15-20, show all other frames
        self.frame_15.hide()
        self.frame_16.hide()
        self.frame_17.hide()
        self.frame_18.hide()
        ...
        # Set initial disabled state for line edits
        ...
        self.checkBox_1.toggled.connect(self.handle_checkbox_change)
        
        # Enable next set based on current values
        self.lineEdit_1.textChanged.connect(self.update_frame8_enabled)
        self.lineEdit_2.textChanged.connect(self.update_frame8_enabled)
        self.lineEdit_3.textChanged.connect(self.update_frame11_enabled)
        self.lineEdit_4.textChanged.connect(self.update_frame11_enabled)
        self.lineEdit_5.textChanged.connect(self.update_pushbutton_enabled)
        self.lineEdit_6.textChanged.connect(self.update_pushbutton_enabled)
    ...
    def handle_checkbox_change(self):
        """Handle checkbox changes to enable first line edits"""
        checked = self.checkBox_1.isChecked()
        
        # Enable first pair of line edits when checkbox is checked
        self.lineEdit_1.setEnabled(checked)
        ...
        if not checked:
            ...
            self.update_frame8_enabled()

    def values_permissible_frame7(self):
        """Check if frame_7 values are permissible (don't trigger error frames)"""
        try:
            v1 = float(self.lineEdit_1.text())
            v2 = float(self.lineEdit_2.text())
            return 122 < v1 < 138 and 122 < v2 < 138
        except ValueError:
            return False

    def values_permissible_frame8(self):
        """Check if frame_8 values are permissible (don't trigger error frames)"""
        try:
            v3 = float(self.lineEdit_3.text())
            v4 = float(self.lineEdit_4.text())
            return -5 < v3 < 5 and -5 < v4 < 5
        except ValueError:
            return False

    def values_permissible_frame11(self):
        """Check if frame_11 values are permissible (don't trigger error frames)"""
        try:
            v5 = float(self.lineEdit_5.text())
            v6 = float(self.lineEdit_6.text())
            return 122 < v5 < 138 and 122 < v6 < 138
        except ValueError:
            return False

    def update_frame8_enabled(self):
        """Enable frame_8 line edits if frame_7 values are permissible"""
        enabled = (self.lineEdit_1.isEnabled() and self.lineEdit_2.isEnabled() and 
                  self.values_permissible_frame7())
        ...
        if not enabled:
            ...
            # Check if we can enable frame_11
            self.update_frame11_enabled()

    def update_frame11_enabled(self):
        """Enable frame_11 line edits if frame_8 values are permissible"""
        enabled = (self.lineEdit_3.isEnabled() and self.lineEdit_4.isEnabled() and 
                  self.values_permissible_frame8())
        self.lineEdit_5.setEnabled(enabled)
        self.lineEdit_6.setEnabled(enabled)
        
        if not enabled:
            # Clear and disable pushButton
            self.lineEdit_5.clear()
            self.lineEdit_6.clear()
            self.pushButton.setEnabled(False)
        else:
            # Check if we can enable pushButton
            self.update_pushbutton_enabled()

    def update_pushbutton_enabled(self):
        """Enable pushButton if frame_11 values are permissible"""
        enabled = (self.lineEdit_5.isEnabled() and self.lineEdit_6.isEnabled() and 
                  self.values_permissible_frame11())
