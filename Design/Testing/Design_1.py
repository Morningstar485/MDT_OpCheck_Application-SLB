...
from .Design_2 import Design_2  # Import the next dialog

class Design_1(QDialog):
    def __init__(self, metadata):
        ...
        self.ui.pushButton_1.clicked.connect(self.confirm_and_open_next)

    def confirm_and_open_next(self):
        # Save metadata from input fields
        self.admin_metadata['Well Name'] = self.ui.lineEdit_1.text()
        self.admin_metadata['Rig Name'] = self.ui.lineEdit_2.text()
        self.admin_metadata['FDP Number'] = self.ui.lineEdit_3.text()

        if not all([
            self.admin_metadata['Well Name'],
            self.admin_metadata['Rig Name'],
            self.admin_metadata['FDP Number']
        ]):
            return  

        # ✅ Proceed to next dialog
        dialog2 = Design_2(self.admin_metadata)
