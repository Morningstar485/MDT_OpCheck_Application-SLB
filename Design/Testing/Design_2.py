...
from .Design_3 import Design_3 # Import the next dialog

class Design_2(QDialog):
    def __init__(self, metadata):
        super().__init__()
        self.admin_metadata = metadata

        self.ui = Ui_Dialog()
        self.ui.setupUi(self)

        # Connect to your confirmation + next dialog logic
        self.ui.pushButton_2.clicked.connect(self.confirm_and_open_next)

    def confirm_and_open_next(self):
        # Collect crew designations
        designations = []
        # Static combo box
        static_designation = self.ui.comboBox.currentText()
        if static_designation and static_designation.lower() != "select":
            designations.append(static_designation)

        # Dynamic combo boxes (if any)
        # Each dynamic row is a QFrame with a QVBoxLayout containing a QComboBox
        if hasattr(self.ui, "dynamicCrewLayout"):
            for i in range(self.ui.dynamicCrewLayout.count()):
                frame = self.ui.dynamicCrewLayout.itemAt(i).widget()
                if frame is not None:
                    # Find QComboBox inside the frame
                    for j in range(frame.layout().count()):
                        combo = frame.layout().itemAt(j).widget()
                        if isinstance(combo, QComboBox):
                            designation = combo.currentText()
                            if designation and designation.lower() != "select":
                                designations.append(designation)

        self.admin_metadata['Crew Designations'] = designations

        # ✅ Proceed to next dialog
        dialog3 = Design_3(self.admin_metadata)
